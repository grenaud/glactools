...
void parallelP<STAT>::launchThreads(string filename,int numberOfThreads,int sizeBins ){
	...
	if( arr->chri !=  chriLast ){  //=UINT16_MAX)
	    ...
	    cerr<<"new chr found, processing chr #: "<<chri2chr[ arr->chri ]<<endl;
	    //(chrS)<<endl;
	}
	
	int currentBin = chrBin+(arr->coordinate/sizeBins);	
	...
	if(lastBin != currentBin){
		...
		cerr<<"processing chr #: "<<(chri2chr[ arr->chri ])<<":"<<arr->coordinate<<endl;
	    }
