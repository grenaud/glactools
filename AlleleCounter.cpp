...
void AlleleCounter::reinitializedCounters(){
    counterSame      =0;
    counterCommon    =0;
    counterReference =0;
    counterSample    =0;
...
double AlleleCounter::lowerConf (const unsigned int shortBranch,const unsigned int commonBranch) const{
...
string AlleleCounter::getHeader(string prefixToAdd){
    return 
	prefixToAdd+"noMut\t"+
	prefixToAdd+"common\t"+
...
double AlleleCounter::highConf (const unsigned int shortBranch,const unsigned int commonBranch) const {
    double pHat = double(shortBranch)/ ( double(shortBranch)+double(commonBranch));
    //1.0 = 85%, 
    //1.6 = 95%
    double z = 1.6;
