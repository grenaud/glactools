...
AvgCoaAlleleCounter::AvgCoaAlleleCounter(){
    reinitializedCounters();
}

void AvgCoaAlleleCounter::reinitializedCounters(){
    counterSame      =0;
    counterCommon    =0;
    counterReference =0;
    counterSample    =0;
...
double AvgCoaAlleleCounter::lowerConf (const unsigned int shortBranch,const unsigned int commonBranch) const{
...
string AvgCoaAlleleCounter::getHeader(string prefixToAdd){
    return 
	prefixToAdd+"noMut\t"+
	prefixToAdd+"common\t"+
...
double AvgCoaAlleleCounter::highConf (const unsigned int shortBranch,const unsigned int commonBranch) const {
    double pHat = double(shortBranch)/ ( double(shortBranch)+double(commonBranch));
    //1.0 = 85%, 
    //1.6 = 95%
    double z = 1.6;
...
//     AvgCoaAlleleCounter al;
//     cout<<al.lowerConf(46,396)<<endl;
// }
double AvgCoaAlleleCounter::avgCoaRefSam () const {
...
double AvgCoaAlleleCounter::avgCoaSamRef () const {
