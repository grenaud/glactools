...
void Vcf2GLF::setVarsEPO(ReadTabix * rtEPO,string & epoChr,unsigned int & epoCoord,bool & cpgEPO,char & allel_chimp,char & allel_anc,bool & lineLeftEPO,string & lineFromEPO){
...
string Vcf2GLF::usage() const{
    ...
    return string(string("") +"Vcf2GLF [options] <vcf file> <name sample> "+"\n"+
		  ...
		  // "\ne.g. Vcf2GLF myfile.vcf.gz Ind1,Ind2,Ind10\n"+                       
...
int Vcf2GLF::run(int argc, char *argv[]){
	...
	if(passedFilters(toprint,filtersVCF)){
		...
		// SingleGL root;
		// SingleGL anc;
		SingleGL sample (MIN(toprint-> getPLHomoRef(),255),
				 MIN(toprint-> getPLHetero(),255),
				 MIN(toprint-> getPLHomoAlt(),255),
				 toprint->isCpg());
		...
		if(!gw->writeAlleleRecord(&arToWrite)){
		    cerr<<"Vcf2GLF: error writing header "<<endl;
		    exit(1);
		}
